...
def evaluate_uncertainty(net,
    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        aleatoric_predictions = torch.empty((0,mask_true.size(0),mask_true.size(1),mask_true.size(2),mask_true.size(3)))
        
        # iterate over the forward passes
        for f_pass in range(nb_forward):

            image_aleatoric = image.clone().detach()
        ...
        aleatoric_std_plot = aleatoric_std.mean(dim=1)
...
if __name__ == '__main__':
    ...
    # define test set
    test_set = BBKDataset(zone = ("genf",), split = "test", buildings = True, vegetation = True, random_seed = 1, augment=False)
